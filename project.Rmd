...
We added columns of features into the business data frame. 

All features of interest are in the attributes column inside the business data frame. We selected reservation, delivery,card,takeout, and parking. There are some other potential features and amenities that can affect the star rating. However, they are a lot of missing values, so we could not use them. 

The attribute column is itself a data frame, so we also used similar approach of creating a temporary dataframe and then later replace the column with the empty column of each feature in the business data frame. They are binary variable, and we replaced some NA values with zero. 
...
Add some text analysis from review data and finalize data wrangling part.
...
We changed the state into binary variable of whether each business ID belongs to the state PA or QC. We deleted the business ID column in order to build predictive models with other features. 
...
We first built the linear regression with all potential predictors, and we used backward selection to choose significant features that relate to star rating. These features will be used to build other prediction models to figure out the most effective model for the prediction of star rating of restaurants. 
...
Another model that we try is the K Nearest Neighbors model. We used cross validation to select the best k first, and we found out that k=19 is the best value. We fitted the model using k=19. 
...
We also tried to build predictive model with support vector machine and random forest. 
...
We calculated the root mean squared error of the predicted star rating and the true rating for each model. 
...
We created the table of RMSE for each model to compare the result. The logistic regression and k-Nearest neighbors do not perform very well. The RMSE of random forest and support vector machine values are smaller. The linear model has the smallest RMSE. 
