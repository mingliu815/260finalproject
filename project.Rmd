...
Basically we successfully investigated our initial questions and reached our goal in this project, while there could be some improvements. First of all, this dataset has a lot of missing values, especially for some features that we think they should be included as the potential variables, such as price range, free wifi or not. In this project, we did not use them at all, while we believe that they would give us more information if we got these values and included them correctly. Furthermore, the location distribution of these restaurants are skewed, and almost all of them are in urban areas (big cities). We think that if we could have more information on rural or other different areas we could glean more insights than what is avaliable on Yelp. 
